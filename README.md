...
* 使用插件管理器（Vundle）：增加配置语句 `Plugin 'xuanfour/vimim_dict'`；
* 手工下载：[Github](https://github.com/xuanfour/vimim_dict) 。点击右下角 `Download ZIP`，解压后把 `vimim_dict.vim` 和字典文件 `vimim_*.txt` 放到 Vim 的 `plugin` 目录下即可；
